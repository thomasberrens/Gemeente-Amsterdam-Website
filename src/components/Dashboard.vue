<template>
  <div class="bg-white min-h-screen p-4">
    <div class="max-w-7xl mx-auto">
      <h1 class="text-4xl font-bold mb-4 text-green-700">Gemeente Amsterdam Dashboard</h1>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <div v-for="(value, key) in choices" :key="key" class="bg-gray-200 rounded-lg shadow-md overflow-hidden" :class="{ 'h-16': !isOpen(key), 'h-auto': isOpen(key) }">
          <div class="p-4">
            <button @click="toggleGraph(key)" class="block font-semibold text-lg mb-2 hover:text-green-700 transition-colors duration-300">{{ value[0].toString() }}</button>
